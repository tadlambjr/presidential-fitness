class DownloadsController < ApplicationController
  def individual_performance_log
    pdf_path = Rails.root.join('public', 'pdfs', 'individual-performance-log.pdf')

    if File.exist?(pdf_path)
      send_file pdf_path, filename: 'individual-performance-log.pdf', type: 'application/pdf', disposition: 'inline'
    else
      # Fallback: generate PDF if file doesn't exist
      generate_individual_pdf
      send_file pdf_path, filename: 'individual-performance-log.pdf', type: 'application/pdf', disposition: 'inline'
    end
  end

  def group_assessment_template
    pdf_path = Rails.root.join('public', 'pdfs', 'group-assessment-template.pdf')

    if File.exist?(pdf_path)
      send_file pdf_path, filename: 'group-assessment-template.pdf', type: 'application/pdf', disposition: 'inline'
    else
      # Fallback: generate PDF if file doesn't exist
      generate_group_pdf
      send_file pdf_path, filename: 'group-assessment-template.pdf', type: 'application/pdf', disposition: 'inline'
  ...
  def generate_individual_pdf
    ...
    pdf.render_file Rails.root.join('public', 'pdfs', 'individual-performance-log.pdf')
  end
  ...
  def generate_group_pdf
    ...
    pdf = Prawn::Document.new(page_layout: :landscape)
    setup_group_pdf(pdf)
    pdf.render_file Rails.root.join('public', 'pdfs', 'group-assessment-template.pdf')
  end
  ...
  def setup_group_pdf(pdf)
    ...
    headers = [
      '',
      'Broad Jump (inches)', 'Pull-ups (reps)', 'Plank (seconds)',
      '50Y Dash (seconds)', 'Vertical Jump (inches)', 'Push-ups (reps)',
      'Toe Reach (inches)', 'Shuttle Run (seconds)', 'Shot Put (inches)',
      'Sit-ups (reps)', 'Wall Sit (seconds)', '1 Mile (minutes)',
      'Chin-ups (reps)', 'Handstand (seconds)', 'Jump Rope (reps)',
      'Rope Climb (seconds)'
    ]
    pdf.table([headers],
      cell_style: {
    ...
    target_headers = [
      'Participant',
      '78/68', '10/2', '180/150', '7.2/7.8', '20/16', '35/20',
      '6/8', '10.5/11.3', '290/240', '50/42', '120/105', '6:30/7:30',
      '12/4', '20/15', '140/125', '12/18'
    ]
